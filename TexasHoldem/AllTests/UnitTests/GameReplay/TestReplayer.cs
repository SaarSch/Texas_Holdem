...
namespace AllTests.UnitTests.GameReplay
{
    ...
    public class TestReplayer
    {
        ...
        [TestMethod]
        public void SaveTurn_SaveValidTurn_TurnSaved()
        {
            var filename = Replayer.CreateReplay();
            var path = Directory.GetCurrentDirectory() + "\\" + filename;
            ...
            Replayer.Save(filename, 5, players, 100, community, "a comment");
            Replayer.Save(filename, 3, players, 300, community, "a comment");
            p2.Folded = true;
            Replayer.Save(filename, 5, players, 400, null, "another comment");
            string turnEntry = "5,fold,150,undef,undef,undef,undef,undef,undef,undef,100,2C;5D;10H;12S;,,,a comment\n" +
                               "5,fold,fold,undef,undef,undef,undef,undef,undef,undef,400,-,,,another comment\n";

            Assert.AreEqual(turnEntry, Replayer.SaveTurn(filename, 5));
            File.Delete(path);
            File.Delete(Directory.GetCurrentDirectory() + "\\gameReplayCounter.txt");
        }

        [TestMethod]
        public void SaveTurn_SaveWithInvalidFile_ExceptionThrown()
        {
            try
            {
        ...
        [TestMethod]
        public void SaveTurn_SaveWithInvalidTurn_ExceptionThrown()
        {
            var filename = Replayer.CreateReplay();
            var p1 = new Player("yossi", new User("KillingHsX", "12345678", "pic.jpg", "hello@gmail.com", 5000));
            var p2 = new Player("kobi", new User("KillingHsX1", "12345678", "pic1.jpg", "hello@gmail.com", 5000));
